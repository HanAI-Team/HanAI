...
:: 첫 실행 (전체 이전)
echo.
echo [2/3] 네오보감 환자 데이터 초기 이전 중...
echo       (처음 실행 시 면허번호와 비밀번호를 입력하세요)
echo.
python "%~dp0neobogam_sync_agent.py"
if errorlevel 1 (
...
schtasks /create /tn "ZinmacNeobogamSync" /tr "python \"%~dp0neobogam_sync_agent.py\"" /sc hourly /f > nul
if errorlevel 1 (
