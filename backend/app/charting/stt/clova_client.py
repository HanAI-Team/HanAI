# app/charting/stt/clova_client.py
"""CLOVA Speech STT 클라이언트 (VITO 전환 전까지 쓰였던 프로덕션 구현).

2026-07-20 VITO(RTZR)로 전환되면서 client.py의 활성 구현 자리를 넘겼다.
VITO에서 예상 못 한 문제가 생기면, chunker.py의 import를
`from app.charting.stt.client import vito_client`에서
`from app.charting.stt.clova_client import clova_client`로 되돌리면 즉시 롤백된다.
"""
import asyncio
import io
import json
