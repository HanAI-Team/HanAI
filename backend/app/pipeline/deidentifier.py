...
class Deidentifier:
    ...
    def __init__(self):
        self.patterns = [
            ...
            # 이름 + 님/씨
            (re.compile(r"[가-힣]{2,4}\s?(?:님|씨)"), "[환자]"),
            # 이름 + 환자분/환자님 (분/님이 붙으면 거의 항상 호칭)
            (re.compile(r"[가-힣]{2,3}\s?환자(?:분|님)"), "[환자]"),
            # 이름 + 환자 단독 (뒤에 조사가 없을 때만 — 일반명사 오탐 방지)
            (re.compile(r"[가-힣]{2,3}\s?환자(?![가는이을도만의들에서로])"), "[환자]"),
            # 환자 뒤 이름
