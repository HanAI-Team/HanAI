...
class BillingInput:
    ...
    exam_fee: int = 0    # 진찰료 (차등수가청구액 계산에 필요)
...
def calculate_billing(inp: BillingInput) -> BillingResult:
    ...
    # 차등수가청구액: 차등지수가 있으면 진찰료에만 차등 적용
    # 공식: 청구액 - {진찰료×(1-차등지수)}. 차등지수는 소수점 여덟째 자리에서
    # 4사5입(반올림)해 일곱째 자리까지 사용 (EDI n(1.7) 저장 형식과 일치).
    if inp.graduated_fee_index > 0:
        index = inp.graduated_fee_index.quantize(
            Decimal("0.0000001"), rounding=ROUND_HALF_UP
        )
        result.graduated_index = index
        reduction = Decimal(inp.exam_fee) * (Decimal("1") - index)
        result.graduated_claim = int(
            (Decimal(result.claim_amount) - reduction).quantize(
