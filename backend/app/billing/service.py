...
async def generate_claim_edi(
    ...
    # 2. ClaimHeader 조립
    hospital_result = await db.execute(select(Hospital).where(Hospital.id == claim.hospital_id))
    hospital = hospital_result.scalar_one_or_none()
    inst_code = hospital.institution_code or "00000000"
    ...
    for i, record in enumerate(medical_records):
        ...
        patient_records.append(PatientRecord(
            ...
            patient_rrn=patient.rrn.replace("-", "") if patient and patient.rrn else "0000000000000",
            inpatient_days=1,
            benefit_days=1,
            benefit_total_1=claim.total_amount,
            copayment=claim.patient_copay,
