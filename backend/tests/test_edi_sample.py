...
from app.billing.service import generate_claim_edi, generate_claim_sam_files
...
# ── EDI 파싱 헬퍼 ─────────────────────────────────────────────────────────────
# 2026-07-11 MCPoS 실측(최소/최대 725바이트) 반영: 한방은 내역구분·처방전발급번호
# 필드가 없다 (「요양급여비용 청구방법...작성요령」 p.137, 한방 "4) 명세서
# 특정내역기재란" 원문 확인).
# (레코드4 특정내역기재란 레이아웃: KEY15 + 발생단위구분1 +
#  줄번호4 + 특정내역구분5 + 특정내역700 = 725바이트)
...
def special_code(line: bytes) -> str:
    """레코드4 특정내역구분코드 (bytes 21-25, 1-indexed → [20:25] 0-indexed)."""
    return line[20:25].decode("euc-kr").strip()


def special_content(line: bytes) -> str:
    """레코드4 특정내역 내용 (bytes 26-725, 1-indexed → [25:725] 0-indexed)."""
    return line[25:725].decode("euc-kr").strip()


def record_line_no(line: bytes) -> int:
    """레코드4 줄번호 (bytes 17-20, 1-indexed → [16:20] 0-indexed)."""
    return int(line[16:20].decode("euc-kr"))
...
async def test_EDI_레코드_수와_길이(db, 한의원_외래_사례):
    ...
    기대 레코드 순서 및 바이트 길이 (2026-07-09 별첨1/EDI 재작성 반영):
      ...
      4  레코드3    초진진찰료            184 bytes
      5  레코드3    투자법침술            184 bytes
      6  레코드4    MT032 접수일시       725 bytes
      7  레코드4    JS010 줄1           725 bytes
      8  레코드4    JS010 줄2           725 bytes
      9  레코드4    JS011 경혈명        725 bytes
    ...
    expected = [2096, 325, 43, 184, 184, 725, 725, 725, 725]
...
async def test_JS011_경혈명_BL060_KI003(db, 한의원_외래_사례):
    ...
    c2_08 = [r for r in parse_lines(edi) if len(r) == 725]
...
# ── 테스트: H010(청구서)·K020.1(명세서) 필드 일치 (MCPoS 요양기관기호/합계 불일치 회귀) ──

async def test_H010_요양기관기호_K020_1과_일치(db, 한의원_외래_사례):
    """청구서(H010)와 명세서(K020.1)의 요양기관기호는 항상 같은 값이어야 한다.

    상시점검(test_mode=True)이라도 SAM 파일 안의 요양기관기호는 실제
    요양기관기호를 써야 한다 — 업체기호는 McPoS 프로그램 자체의 로컬
    환경설정에만 기재하는 값이지 SAM 파일 내용이 아니다(「청구소프트웨어
    신규검사 신청방법」 2025.7판 23·26페이지, 2026-07-16 확인).
    """
    claim, hospital = 한의원_외래_사례
    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010, k0201 = files["H010"], files["K020.1"]
    assert h010[20:28] == k0201[19:27] == hospital.institution_code.encode("euc-kr")


async def test_같은_병원_같은_달_두번째_청구서는_다른_청구번호(db, 한의원_외래_사례, approved_doctor):
    """보완·추가청구 재전송, 상시점검 재시험 등으로 같은 병원·같은 진료년월에
    청구서를 다시 만들면 청구번호가 겹치면 안 된다(2026-07-16 확인)."""
    claim, hospital = 한의원_외래_사례
    doctor, _ = approved_doctor

    r_patient = await db.execute(select(Patient).where(Patient.id == claim.patient_id))
    patient = r_patient.scalar_one()

    second_record = MedicalRecord(
        patient_id=patient.id,
        doctor_id=doctor.id,
        hospital_id=hospital.id,
        kcd_code="M5459",
        chart_structured="요통 재청구",
        recorded_at=VISIT_DT,
        status="completed",
    )
    ...
    db.add(second_claim)
    second_record.claim_id = second_claim.id
    await db.flush()
    db.add(ClaimLineItem(
        claim_id=second_claim.id,
        medical_record_id=second_record.id,
        hang="04", mok="01",
        code="40080010",
        name="투자법 침술(야간)",
        unit_price=4220, qty=1, days=1, amount=4220,
    ...
    first_files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    second_files = await generate_claim_sam_files(db, hospital.id, second_claim.id, test_mode=True)

    first_no = first_files["H010"][6:16]
    second_no = second_files["H010"][6:16]
    assert first_no != second_no
    assert first_no == b"2026060001"
    assert second_no == b"2026060002"


async def test_H010_작성자생년월일_의사_생년월일_반영(db, 한의원_외래_사례, approved_doctor):
    """작성자생년월일(294-306, an(13))은 doctor.birth_date를 YYYYMMDD로 채우고
    나머지는 공백 패딩한다 — 숫자 필드처럼 0으로 zfill하면 안 된다(2026-07-16 확인)."""
    claim, hospital = 한의원_외래_사례
    doctor, _ = approved_doctor
    doctor.birth_date = date(1975, 3, 20)
    await db.commit()

    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010 = files["H010"]
    field = h010[293:306].decode("euc-kr")
    assert field == "19750320     "


async def test_H010_작성자생년월일_없으면_공백(db, 한의원_외래_사례):
    """doctor.birth_date가 없으면(기본 픽스처) 13바이트 전부 공백."""
    claim, hospital = 한의원_외래_사례
    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010 = files["H010"]
    field = h010[293:306].decode("euc-kr")
...
# ── 차등수가 진료(조제)일수·지수·청구액 (MCPoS 반송 33-01/07 회귀, 2026-07-16) ──

async def test_H010_차등수가_진료일수는_명세서_1건이면_1(db, 한의원_외래_사례):
    """반송사유 33-01(진료일수가 0인 경우) 회귀 — 명세서 1건짜리 청구서는
    진료일수가 1로, 차등지수는 1.0000000(미적용)으로 나가야 한다."""
    claim, hospital = 한의원_외래_사례
    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010 = files["H010"]
    assert h010[215:221].decode("euc-kr") == "000100"  # 차등수가 진료(조제)일수 = 1.00
    assert h010[225:233].decode("euc-kr") == "10000000"  # 차등지수 = 1.0000000
    assert int(h010[233:245].decode("euc-kr")) == claim.claim_amount  # 차등수가청구액


async def test_H010_차등수가_진료일수는_서로다른_진료일자_수(db, 한의원_외래_사례, approved_doctor):
    """명세서가 2건이고 진료일자가 다르면 진료일수는 2, 1일 평균 진찰횟수는 1이라
    기준(75) 이내라 그대로 차등지수 1.0000000이 나가야 한다."""
    claim, hospital = 한의원_외래_사례
    doctor, _ = approved_doctor

    other_day_record = MedicalRecord(
        patient_id=claim.patient_id,
        doctor_id=doctor.id,
        hospital_id=hospital.id,
        kcd_code="M5459",
        chart_structured="요통 재진",
        recorded_at=datetime(2026, 6, 15, 10, 0, tzinfo=timezone.utc),
        claim_id=claim.id,
        status="completed",
    )
    db.add(other_day_record)
    await db.flush()
    db.add(ClaimLineItem(
        claim_id=claim.id,
        medical_record_id=other_day_record.id,
        hang="01", mok="01",
        code="10110011",
        name="재진진찰료",
        unit_price=7160, qty=1, days=1, amount=7160,
    ))
    await db.commit()

    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010 = files["H010"]
    assert h010[215:221].decode("euc-kr") == "000200"  # 진료일수 = 2.00
    assert h010[225:233].decode("euc-kr") == "10000000"  # 1일 평균 1회 -> 미적용


async def test_H010_1일평균진찰횟수_기준초과시_생성차단(db, 한의원_외래_사례, approved_doctor):
    """차등지수 구간별 지수표가 없어, 기준(75회)을 넘으면 값을 만들어내지 않고
    막는다 — 반송사유 33-07을 피하겠다고 틀릴 수 있는 값을 넣는 것보다 안전."""
    claim, hospital = 한의원_외래_사례
    doctor, _ = approved_doctor
    ...
    with pytest.raises(HTTPException) as exc_info:
        await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    assert exc_info.value.status_code == 400
    assert "차등지수" in exc_info.value.detail


async def test_H010_합계필드_명세서_1건_합산과_일치(db, 한의원_외래_사례):
    """H010의 합계 4개 필드는 K020.1(1건)의 대응 필드와 같아야 한다."""
    claim, hospital = 한의원_외래_사례
    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    h010, k0201 = files["H010"], files["K020.1"]

    assert int(h010[119:131]) == int(k0201[245:255])  # 요양급여비용총액2
    assert int(h010[167:179]) == int(k0201[285:295])  # 100분의100미만총액
    assert int(h010[179:191]) == int(k0201[295:305])  # 100분의100미만본인일부부담금
    assert int(h010[191:203]) == int(k0201[305:315])  # 100분의100미만청구액
    # 실제로 0이 아닌 값으로 채워지는지도 함께 확인 (전부 0이면 위 assert가 거짓양성으로 통과함)
    assert int(h010[119:131]) > 0


async def test_H010_합계필드_명세서_2건_합산과_일치(db, 한의원_외래_사례, approved_doctor):
    """명세서(K020.1)가 2건이면 H010은 두 명세서 값의 합이어야 한다."""
    claim, hospital = 한의원_외래_사례
    doctor, _ = approved_doctor
    ...
    record2 = MedicalRecord(
        ...
        claim_id=claim.id,
        kcd_code="M5459",
        chart_structured="요통, 상세불명의 부위 (재진)",
        recorded_at=VISIT_DT,
        status="completed",
    )
    db.add(record2)
    ...
    files = await generate_claim_sam_files(db, hospital.id, claim.id, test_mode=True)
    k0201_lines = [
        files["K020.1"][i:i + 325] for i in range(0, len(files["K020.1"]), 327)
    ]
    assert len(k0201_lines) == 2, f"명세서 수 불일치: {len(k0201_lines)}"

    expected_total2 = sum(int(line[245:255]) for line in k0201_lines)
    h010 = files["H010"]
    assert int(h010[119:131]) == expected_total2
    assert expected_total2 > 0
