...
2026-07-09 전면 재계산: edi_writer.py가 별첨2(전산매체 작성요령) 원문 대조로
전면 재작성되면서(RecordKey는 유지, 자료구분/발생단위구분 등 필드 신설,
레코드별 바이트 길이 전부 변경) 이 파일의 모든 오프셋/길이 기댓값을 다시
계산했다. 특히 C2-00(345)과 C2-11(271) 길이가 이제 서로 달라져서, 기존에
"345바이트 레코드 중 2번째"로 C2-11을 골라내던 우회 로직이 필요 없어졌다
(길이만으로 유일하게 구분 가능).
"""
...
# ── EDI 파싱 헬퍼 ─────────────────────────────────────────────────────────────
# 2026-07-09 별첨2 원문 재검증 반영 — 오프셋 전면 재계산.
# (C2-08 명세서특정내역 레이아웃: KEY17 + 자료구분1 + 발생단위구분1 +
#  처방전발급번호13 + 진료내역확장번호4 + 특정내역구분5 + 특정내역700 = 741바이트)
...
def special_code(line: bytes) -> str:
    """C2-08 특정내역구분코드 (bytes 37-41, 1-indexed → [36:41] 0-indexed)."""
    return line[36:41].decode("euc-kr").strip()


def special_content(line: bytes) -> str:
    """C2-08 특정내역 내용 (bytes 42-741, 1-indexed → [41:741] 0-indexed)."""
    return line[41:741].decode("euc-kr").strip()


def record_ext_no(line: bytes) -> int:
    """C2-08 진료내역확장번호 = 줄번호 (bytes 33-36, 1-indexed → [32:36] 0-indexed)."""
    return int(line[32:36].decode("euc-kr"))
...
async def test_EDI_레코드_수와_길이(db, 한의원_외래_사례):
    """
    기대 레코드 순서 및 바이트 길이 (2026-07-09 별첨2 재검증 반영):
      1  C2-00  심사청구서         345 bytes
      2  C2-11  명세서일반내역      271 bytes
      3  C2-02  명세서상병내역       51 bytes
      4  C2-13  초진진찰료         185 bytes
      5  C2-13  투자법침술         185 bytes
      6  C2-08  MT032 접수일시     741 bytes
      7  C2-08  JS010 줄1         741 bytes
      8  C2-08  JS010 줄2         741 bytes
      9  C2-08  JS011 경혈명      741 bytes
     10  C2-09  EOF                18 bytes
    """
    ...
    expected = [345, 271, 51, 185, 185, 741, 741, 741, 741, 18]
...
async def test_JS011_경혈명_BL060_KI003(db, 한의원_외래_사례):
    ...
    c2_08 = [r for r in parse_lines(edi) if len(r) == 741]
...
# 2026-07-09: C2-00(345)과 C2-11(271) 길이가 이제 서로 달라서, 길이만으로
# C2-11을 바로 고유하게 골라낼 수 있음 (기존의 "[1]" 인덱스 우회 불필요).
# 접수번호(249-255,1-idx→[248:255]) / 명일련(256-260→[255:260]) / 사유(261-262→[260:262])
...
async def test_보완청구_접수번호_명일련_사유코드_반영(db, 한의원_외래_사례):
    ...
    edi = await generate_claim_edi(db, hospital.id, claim.id)
    c2_11_list = [r for r in parse_lines(edi) if len(r) == 271]
    assert len(c2_11_list) == 1, f"C2-11 레코드 수 불일치: {len(c2_11_list)}"
    c2_11 = c2_11_list[0]

    assert c2_11[248:255].decode("euc-kr") == "1234567"  # 249-255 접수번호
    assert c2_11[255:260].decode("euc-kr") == "00005"    # 256-260 명일련
    assert c2_11[260:262].decode("euc-kr") == "12"        # 261-262 사유
...
async def test_최초청구는_접수번호_공란(db, 한의원_외래_사례):
    ...
    assert c2_11[248:255].decode("euc-kr") == "0000000"
    assert c2_11[255:260].decode("euc-kr") == "00000"
    assert c2_11[260:262].decode("euc-kr") == "  "
